...
-- [回答]
 ...
 INNER JOIN store_table AS b ON a.store_code = b.store_code
 INNER JOIN goods_table AS c ON a.goods_code = c.goods_code
 
-- 4. 商品テーブルから全商品の単価の平均値を抽出しなさい。
-- [回答]
SELECT round(AVG(price)) AS "平均単価"
FROM goods_table
 
-- 5. 店舗コード='EA01'の在庫数の平均値より大きい在庫数を持つ店舗コードを抽出しなさい。
-- [回答]
SELECT store_code AS "店舗コード"
FROM stock_table
GROUP BY store_code
HAVING SUM(quantity) > (
SELECT AVG(quantity)
...
-- [回答]
UPDATE stock_table 
SET quantity = 10,update_day = current_timestamp
